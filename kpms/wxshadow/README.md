...
- AUTO TLB flush 会先调用可用的内核 helper，再补一次 VA-all-ASID broadcast TLBI；这比精确 ASID flush 稍重，但能覆盖部分 vendor kernel 在 KPM 调用上下文下 helper 未刷新当前用户执行翻译的情况。
- runtime info 日志降噪为 `wx_info()`，默认静默，保留错误和警告便于定位真正失败。

## 2026-06 隐藏覆盖面加强

- GUP hiding 现在要求至少成功 hook `follow_page_pte` 或 `follow_page_mask`。如果两个符号都不存在或 hook 失败，模块拒绝加载，避免 `/proc/<pid>/mem`、`process_vm_readv`、`ptrace` 等路径看到 shadow PFN。
- `follow_page_mask` 用作旧内核/vendor 内核 fallback；优先使用 `follow_page_pte`，不可用时再挂 `follow_page_mask`。
- 新增可选 `handle_mm_fault` hook，用于 IOPF/SVA/device fault 读路径：读 fault 临时切到原始页，写 fault 触发逻辑释放，指令 fault 尝试恢复 shadow。
- 符号解析会自动识别 3 参数或 4 参数 `kallsyms_on_each_symbol` callback 形式；无法使用 callback 遍历时才回退到 `kallsyms_lookup_name`。
...
- 每个断点最多 4 个寄存器修改
- 运行端必须能 hook `follow_page_pte` 或 `follow_page_mask` 之一，否则模块拒绝加载
- 需要 KernelPatch 框架支持
...
| `handle_mm_fault` | hook (可选) | IOPF/SVA/device fault 读路径隐藏 |
| `follow_page_pte` | hook (优先，必需其一) | GUP 隐藏 |
| `follow_page_mask` | hook (fallback，必需其一) | GUP 隐藏 |
| `dup_mmap` / `uprobe_dup_mmap` | hook (可选) | fork 保护 |
