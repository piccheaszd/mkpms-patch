...
`anti-detect` 1.2.23 起提供 per-mm profile ABI，后续可让目标进程显式注册检测策略。已注册进程优先按 profile flags 处理。1.2.24 起，单独设置 `AD_F_AUDIT_ONLY` 时只注册轻量 profile，不修改 syscall 结果或用户缓冲区；如果同时设置具体检测面 flag，则只统计“本来会被处理”的事件，并在 release / `exit_mmap` 时通过 dmesg 输出计数。1.2.26 起默认启用 profile-only strict：未注册进程不再走旧的 UID/进程名 fallback，只有 root 可通过 `PR_ANTIDETECT_SET_MODE` 临时恢复 legacy fallback。`TracerPid` 处理已改为 fd-aware 并带 per-mm fd cache：只有确认 read fd 指向 `/proc/.../status` 时才扫描并过滤，避免误改普通文件内容，也避免每次 read 都重复 `fget` / `d_path`。1.2.33 起移除 1.2.31 的 PAIC comm 自动兼容，并修正 1.2.32 仍依赖当前线程 comm 的问题：PAIC 常驻只能通过显式 UID allowlist 启用 `paic-stable`，保持 profile-only strict，不靠线程名自动触发，也不因 loader/Binder/JIT 线程名变化而失效。1.2.34 起将 `add-uid-paic` 收敛为 daily light：只启用路径、fd-link、目录隐藏和 `libxloader.so` one-shot exit，不再默认伪造 `TracerPid`、`PTRACE_TRACEME` 或 `PR_GET_DUMPABLE`；完整隐藏面改为显式 `add-uid-paic-full`。1.2.35 起 PAIC mmap seed 只创建 rule-only profile，不把 PAIC UID 隐藏面写入 per-mm profile，并为 feature profile / fd cache 增加热路径快速返回，避免常驻后所有 hooked syscall 都被 profile 查询拖慢。1.2.36 起未注册 UID 在 profile-only 下会先做 lockless allowlist 预检查，避免微信等未注册 App 的 hooked syscall 进入 PAIC flag 解析和 persistent UID 锁。1.2.37 起 `ad_feature_profile_count` 只统计 `AD_F_FEATURE_MASK` 中的实际检测面，不再把纯 `AD_F_AUDIT_ONLY` profile 算作 feature profile；即使 observe-only / baseline 会话异常残留 audit-only profile，也不会让全局 hooked syscall 进入 `get_task_mm + profile lock` 热路径。1.2.38 起纯 audit-only profile 不再接收 self-protect rule，persistent rule-only profile 仍保留给 PAIC stable 使用，避免 BOCHK/RF 轻量探针被后续 executable mmap seeding 改造成 active rule 容器。
...
prctl(0x41440005, uid, 0, 0, 0);            /* PR_ANTIDETECT_ADD_PERSISTENT_UID; root only */
prctl(0x41440006, 0, 0, 0, 0);              /* PR_ANTIDETECT_CLEAR_PERSISTENT_UIDS; root only */
```

仓库同时构建 `anti_detect_ctl` 用户态辅助工具，便于在目标启动前由 root 预注册 UID allowlist：

```sh
anti_detect_ctl add-uid 10234
anti_detect_ctl add-uid-active 10234
anti_detect_ctl add-uid-paic 10234
anti_detect_ctl add-uid-paic-full 10234
anti_detect_ctl add-uid-paic-hide 10234
anti_detect_ctl clear-uids
anti_detect_ctl mode profile-only
```

`add-uid` 只启用 seed-only；`add-uid-paic` 启用 PAIC 日常常驻 daily 模式：路径、fd-link、目录隐藏常驻，命中 `libxloader.so` 后只注册 `exit + LR low16 == 0xe120 + one-shot` 兜底，不启用 kill blocking，也不伪造 `TracerPid`、`PTRACE_TRACEME` 或 `PR_GET_DUMPABLE`。`add-uid-paic-full` 恢复完整隐藏面 + one-shot exit，仅用于短时对比诊断；`add-uid-paic-hide` 只开 light hide，不拦 exit，用于确认 PAIC 是否仍会自杀退出。`add-uid-active` 会让 KPM 在命中 loader 可执行映射后自动开启 `AD_F_BLOCK_SELF_EXIT | AD_F_BLOCK_SELF_KILL`，仅用于确认目标不会重试自杀的短时测试，不适合作为 PAIC 日常常驻。

PAIC daily 可用 `kpms/anti-detect/paic_stable_service.sh` 常驻自动注册。将脚本安装为 `/data/adb/service.d/paic_stable.sh` 并 `chmod 0755`，脚本会在开机后动态查询 `com.paic.mo.client` UID，再执行 `anti_detect_ctl mode profile-only` 和 `anti_detect_ctl add-uid-paic <uid>`。脚本默认查找 `/data/local/tmp/anti_detect_ctl`，也支持 `/data/adb/kp-next/bin/anti_detect_ctl`。
...
如果 flags 为 `0`，KPM 默认启用全部检测面；只设置 `AD_F_AUDIT_ONLY` 不再自动扩成全检测面，适合作为 BOCHK 等强对抗目标的轻量注册探针。1.2.37 起纯 audit-only profile 也不再计入 feature profile 热路径计数；1.2.38 起纯 audit-only profile 也不会接收 self-protect rule。需要 audit-only + 全检测面时显式传 `0x1ff`。profile 绑定调用进程的 `mm`，模块会在 `exit_mmap` 清理，也可由目标进程内 `PR_ANTIDETECT_RELEASE` 主动释放。

active self-protect 规则也绑定当前注册 profile/mm。1.2.26 起 `exit` / `kill` blocking 不再按通用 caller path token 判断，只匹配预注册 exact range；1.2.31 的 PAIC 内置兼容已废弃。1.2.35 的常驻 mmap seeding 只提前把命中的 loader 可执行映射注册成 exact range，不恢复旧的全局 caller path fallback，也不为 PAIC daily/full 隐藏面创建 feature profile。PAIC 的 `libxloader.so` 例外必须由 `add-uid-paic <uid>` 显式启用。daily 模式只开启 light hide + `AD_F_BLOCK_SELF_EXIT`，不启用 `AD_F_BLOCK_SELF_KILL`；full 模式才会额外启用 `TracerPid`、ptrace 和 dumpable 伪装。PAIC stable 以显式 UID 为准，不再要求当前线程 comm 匹配 `com.paic` / `mo.client`。注册的 PAIC rule 是 `exit + LR + LR low16 == 0xe120 + one-shot`，命中一次后自动失效，后续异常退出放行，避免 loader 在异常路径里被反复拦截。rule flags 可组合：

- `0x001`：匹配 exit
- `0x002`：匹配 kill
- `0x004`：PC 落在 range 内
- `0x008`：LR 落在 range 内
- `0x010`：PAIC 兼容规则，仅匹配 `LR & 0xffff == 0xe120`
- `0x020`：one-shot，命中后自动删除该 rule

rustFrida loader 会在启用 `AD_F_BLOCK_SELF_EXIT` / `AD_F_BLOCK_SELF_KILL` 时注册 stage-1 RX 和 linker veneer range。需要在 rustFrida 启动前保留 loader range（例如 PAIC `libxloader.so`）应走 UID allowlist；PAIC 日常只使用 `add-uid-paic`，不要使用 `mode legacy` 或 `add-uid-active`。除 PAIC 的 one-shot LR 兼容分支外，真正启用 active blocking 仍应由 RF profile 或显式 `add-uid-active` 控制。没有注册 exact range 的 App 即使打开了 active flags，也不会因为旧的 loader-token 规则被误拦截。
