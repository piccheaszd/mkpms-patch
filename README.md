...
如果 flags 为 `0`，KPM 默认启用全部检测面；只设置 `AD_F_AUDIT_ONLY` 不再自动扩成全检测面，适合作为 BOCHK 等强对抗目标的轻量注册探针。1.2.37 起纯 audit-only profile 也不再计入 feature profile 热路径计数。需要 audit-only + 全检测面时显式传 `0x1ff`。profile 绑定调用进程的 `mm`，模块会在 `exit_mmap` 清理，也可由目标进程内 `PR_ANTIDETECT_RELEASE` 主动释放。
