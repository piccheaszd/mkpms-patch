...
`add-uid` 只启用 seed-only；`add-uid-active` 会让 KPM 在命中 loader 可执行映射后自动开启 `AD_F_BLOCK_SELF_EXIT | AD_F_BLOCK_SELF_KILL`，仅用于确认目标不会重试自杀的短时测试。
...
active self-protect 规则也绑定当前注册 profile/mm。1.2.26 起 `exit` / `kill` blocking 不再按通用 caller path token 判断，只匹配预注册 exact range；1.2.31 的常驻 mmap seeding 只提前把命中的 loader 可执行映射注册成 exact range，不恢复旧的全局 caller path fallback。PAIC 的 `libxloader.so` 例外会同时启用完整检测面 flags 和旧版实测过的 `LR & 0xffff == 0xe120` exit 兼容规则，但仍不会恢复 1.2.25 的全局动态 caller path 解析。rule flags 可组合：
